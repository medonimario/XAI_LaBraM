...
if __name__ == "__main__":
    ...
    raw_dataset_path = os.getenv("MG_DATASET_PATH") + "preprocessed_synchrony/"

    # Filter for .set files and only include 'SpontaneousNoSynchro' or 'SpontaneousSynchro' conditions
    all_files = [
        f for f in os.listdir(raw_dataset_path) 
        if f.endswith(".set") and ('SpontaneousNoSynchro' in f or 'SpontaneousSynchro' in f)
    ]
    ...
    train_dump_folder = os.path.join(final_root, "processed_synchrony", "train")
    val_dump_folder = os.path.join(final_root, "processed_synchrony", "val")
    test_dump_folder = os.path.join(final_root, "processed_synchrony", "test")
    ...
    def prepare_params(file_list, dump_folder):
        ...
        for f in file_list:
            ...
            # Determine label from filename: 0 for SpontaneousNoSynchro, 1 for SpontaneousSynchro
            label = 1 if 'SpontaneousSynchro' in f else 0
